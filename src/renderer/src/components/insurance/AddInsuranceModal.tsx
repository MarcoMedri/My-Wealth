...
export function AddInsuranceModal({ isOpen, onClose, initialData }: AddInsuranceModalProps) {
    const { accounts, brokers, saveInsurance, deleteInsurance, refreshData } = useVaultStore();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!name || !premiumAmount || !brokerId) return;
                ...
                <div className="p-6 text-center space-y-4">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-foreground">
                            {t('insurance.noBrokerTitle') || 'No Broker Found'}
                        </h3>
                        <p className="text-foreground-muted mt-2">
                            {t('insurance.noBrokerDesc') || 'You need to create a Broker before adding an Insurance Policy.'}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground-muted mb-1">
                                    {t('insurance.linkedAccount') || 'Linked Account'}
                                </label>
                                <select
                                    className="w-full px-3 py-2 bg-background-subtle border border-border rounded-lg text-foreground focus:ring-2 focus:ring-primary outline-none"
                                    value={accountId}
